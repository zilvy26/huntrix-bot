...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt => opt.setName('category').setDescription('Filter by category (kpop, anime, game; comma to OR)'))
    ...
    .addStringOption(opt => opt.setName('name').setDescription('Filter by name (comma to OR)'))
  ...
  async execute(interaction) {
    ...
    const cardQuery = {};
    if (categories.length) cardQuery.category = { $in: categories };
    if (groups.length)     cardQuery.group = { $in: groups };
    if (eras.length)       cardQuery.era = { $in: eras };
    if (names.length)      cardQuery.name = { $in: names };
